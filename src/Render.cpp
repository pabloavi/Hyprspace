...
public:
  ...
  CFakeDamageElement(const CBox &box) { this->box = box; }
  virtual ~CFakeDamageElement() = default;

  virtual void draw(const CRegion &damage) { return; }
  ...
  virtual const char *passName() { return "CFakeDamageElement"; }
};
...
void renderWindowStub(PHLWINDOW pWindow, PHLMONITOR pMonitor,
                      ...
                      timespec *time) {
  if (!pWindow || !pMonitor || !pWorkspaceOverride || !time)
    return;
  ...
  renderModif.modifs.push_back(
      {SRenderModifData::eRenderModifType::RMOD_TYPE_TRANSLATE,
       (pMonitor->m_position * pMonitor->m_scale) +
           (rectOverride.pos() / curScaling) -
           (oRealPosition * pMonitor->m_scale)});
  renderModif.modifs.push_back(
      {SRenderModifData::eRenderModifType::RMOD_TYPE_SCALE, curScaling});
  ...
  pWindow->m_windowData.rounding = CWindowOverridableVar<Hyprlang::INT>(
      pWindow->rounding() * curScaling * pMonitor->m_scale,
      eOverridePriority::PRIORITY_SET_PROP);
  g_pInputManager->m_currentlyDraggedWindow =
      pWindow; // override these and force INTERACTIVERESIZEINPROGRESS = true to
               // trick the renderer
  g_pInputManager->m_dragMode = MBIND_RESIZE;

  g_pHyprRenderer->m_renderPass.add(makeUnique<CRendererHintsPassElement>(
      CRendererHintsPassElement::SData{renderModif}));
  // remove modif as it goes out of scope (wtf is this blackmagic i need to
  // relearn c++)
  ...
  (*(tRenderWindow)pRenderWindow)(g_pHyprRenderer.get(), pWindow, pMonitor,
                                  time, true, RENDER_PASS_ALL, false, false);
...
void renderLayerStub(PHLLS pLayer, PHLMONITOR pMonitor, CBox rectOverride,
                     timespec *time) {
  if (!pLayer || !pMonitor || !time)
    return;

  if (!pLayer->m_mapped || pLayer->m_readyToDelete || !pLayer->m_layerSurface)
    return;
  ...
  renderModif.modifs.push_back(
      {SRenderModifData::eRenderModifType::RMOD_TYPE_TRANSLATE,
       pMonitor->m_position + (rectOverride.pos() / curScaling) -
           oRealPosition});
  renderModif.modifs.push_back(
      {SRenderModifData::eRenderModifType::RMOD_TYPE_SCALE, curScaling});
  renderModif.enabled = true;
  ...
  (*(tRenderLayer)pRenderLayer)(g_pHyprRenderer.get(), pLayer, pMonitor, time,
                                false);
...
void CHyprspaceWidget::draw() {
  ...
  if (Config::panelBorderWidth > 0) {
    // Border box
    CBox borderBox = {widgetBox.x,
                      owner->m_position.y +
                          (Config::onBottom * owner->m_transformedSize.y) +
                          (Config::panelHeight + Config::reservedArea -
                           curYOffset->value() * owner->m_scale) *
                              bottomInvert,
                      owner->m_transformedSize.x, Config::panelBorderWidth};
  ...
  // CBox damageBox = {0, (Config::onBottom * (owner->m_transformedSize.y -
  // ((Config::panelHeight + Config::reservedArea)))) - (bottomInvert *
  // curYOffset->value()), owner->m_transformedSize.x, (Config::panelHeight +
  // Config::reservedArea) * owner->m_scale};

  // owner->addDamage(damageBox);
  g_pHyprRenderer->damageMonitor(owner);

  // add a fake element with needsliveblur = true and covers the entire monitor
  // to ensure damage applies to the entire monitor unoptimized atm but hey its
  // working
  CFakeDamageElement fakeDamage =
      CFakeDamageElement(CBox({0, 0}, owner->m_transformedSize));
  ...
  double workspaceGroupWidth =
      workspaceBoxW * wsCount +
      (Config::workspaceMargin * owner->m_scale) * (wsCount - 1);
  double curWorkspaceRectOffsetX =
      Config::centerAligned
          ? workspaceScrollOffset->value() + (widgetBox.w / 2.) -
                (workspaceGroupWidth / 2.)
          : workspaceScrollOffset->value() + Config::workspaceMargin;
  double curWorkspaceRectOffsetY =
      !Config::onBottom ? (((Config::reservedArea + Config::workspaceMargin) *
                            owner->m_scale) -
                           curYOffset->value())
                        : (owner->m_transformedSize.y -
                           ((Config::reservedArea + Config::workspaceMargin) *
                            owner->m_scale) -
                           workspaceBoxH + curYOffset->value());
  double workspaceOverflowSize =
      std::max<double>(((workspaceGroupWidth - widgetBox.w) / 2) +
                           (Config::workspaceMargin * owner->m_scale),
                       0);
  ...
  for (auto wsID : workspaces) {
    ...
    if (ws == owner->m_activeWorkspace) {
      ...
      if (!Config::drawActiveWorkspace) {
        curWorkspaceRectOffsetX +=
            workspaceBoxW + (Config::workspaceMargin * owner->m_scale);
        continue;
      }
    } else {
      if (Config::workspaceBorderSize >= 1 &&
    ...
    if (!Config::hideBackgroundLayers) {
      for (auto &ls : owner->m_layerSurfaceLayers[0]) {
        CBox layerBox = {curWorkspaceBox.pos() +
                             (ls->m_realPosition->value() - owner->m_position) *
                                 monitorSizeScaleFactor,
                         ls->m_realSize->value() * monitorSizeScaleFactor};
    ...
    if (owner->m_activeWorkspace == ws && Config::affectStrut) {
      ...
      if (Config::onBottom)
        miniPanelBox = {curWorkspaceRectOffsetX,
                        curWorkspaceRectOffsetY + workspaceBoxH -
                            widgetBox.h * monitorSizeScaleFactor,
                        widgetBox.w * monitorSizeScaleFactor,
                        widgetBox.h * monitorSizeScaleFactor};
    ...
    if (ws != nullptr) {
      ...
      for (auto &w : g_pCompositor->m_windows) {
        ...
        if (w->m_workspace == ws && !w->m_isFloating) {
          ...
          g_pHyprOpenGL->m_renderData.clipBox = curWorkspaceBox;
          // g_pHyprOpenGL->renderRectWithBlur(&curWindowBox, CHyprColor(0, 0,
          // 0, 0));
