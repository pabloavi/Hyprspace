...
CHyprspaceWidget::CHyprspaceWidget(uint64_t inOwnerID) {
  ...
  *curAnimationConfig.pValues.lock() = curAnimation;
  ...
  g_pAnimationManager->createAnimation(
      0.F, curYOffset, curAnimationConfig.pValues.lock(), AVARDAMAGE_ENTIRE);
  g_pAnimationManager->createAnimation(0.F, workspaceScrollOffset,
    ...
    for (auto &ws : g_pCompositor->m_workspaces()) {
      if (ws->m_monitor->m_id == ownerID) {
        ...
        if (w != nullptr && ws->m_fullscreenMode != FSMODE_NONE) {
          ...
          // we use the getWindowFromHandle function to prevent dangling
          // pointers
          prevFullscreen.emplace_back(
              std::make_tuple((uint32_t)(((uint64_t)w.get()) & 0xFFFFFFFF),
                              ws->m_fullscreenMode));
...
void CHyprspaceWidget::hide() {
  ...
  for (auto &ls : owner->m_layerSurfaceLayers[2]) {
    if (!ls->m_readyToDelete && ls->m_mapped && ls->m_fadingOut) {
      auto oAlpha = std::find_if(
          oLayerAlpha.begin(), oLayerAlpha.end(),
          [&](const auto &tuple) { return std::get<0>(tuple) == ls; });
  ...
  for (auto &fs : prevFullscreen) {
    ...
    g_pCompositor->setWindowFullscreenState(w,
                                            SFullscreenState(oFullscreenMode));
    if (oFullscreenMode == FSMODE_FULLSCREEN)
      w->m_wantsInitialFullscreen = false;
  }
